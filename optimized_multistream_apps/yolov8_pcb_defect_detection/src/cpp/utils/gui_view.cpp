...
DisplayScreen::~DisplayScreen()
{
    ...
    {
        switchTimer_->stop();
        switchTimer_->deleteLater();
        switchTimer_ = nullptr;
    }
}
...
void DisplayScreen::ShowNumViewers(uint32_t num_channels)
{
    if (viewers_.empty()) {
        return;
    }

    // Reset layout base on num_channels
    std::vector<ViewerGeometry> viewer_geometry;
    int screen_width = this->width();
    int screen_height = this->height();
    {
    ...
    for (uint32_t idx = 0; idx < num_channels; idx++){
        ViewerGeometry &cg = viewer_geometry.at(idx);
        FrameViewer *viewer = viewers_[idx];
        viewer->SetGeometry(cg.x, cg.y, cg.w, cg.h);
        viewer->show();
    }
...
void DisplayScreen::EnableSwitchTimer(int interval_seconds) {
    ...
    connect(switchTimer_, &QTimer::timeout, this, [this]() mutable {
        static bool switch_to_1ch = false;

        if (switch_to_1ch) {
            this->ShowNumViewers(this->num_viewers_);
        } else {
            this->ShowNumViewers(1);
        }

        switch_to_1ch = !switch_to_1ch;
    });

    switchTimer_->start(interval_seconds * 1000);
}
...
void FrameViewer::SetGeometry(int x, int y, int w, int h)
{
    ...
    if (!this->display_frame_list_.empty())
        return;
...
void FrameViewer::SlotUpdateFrame(cv::Mat *frame)
{
    ...
    QPixmap pixmap = QPixmap::fromImage(img).scaled(this->width(), this->height(), Qt::KeepAspectRatio, Qt::FastTransformation);
    frame_->setPixmap(pixmap);
}
