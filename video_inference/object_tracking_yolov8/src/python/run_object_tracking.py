...
class App:
    ...
    def process_model_output(self, *ofmaps):
        ...
        for xyxy, tid, cls in zip(tracks.xyxy, tracks.tracker_id, tracks.class_id):
            x1, y1, x2, y2 = map(int, xyxy)
            box_color = colors(cls, bgr=True)  # color for each class.
            ...
            cv.polylines(img, [points], isClosed=False, color=box_color,
            ...
            label = f"{COCO_CLASSES[int(cls)]}#{tid}"
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Run MX3 real-time object tracking using YOLOv8 and ByteTrack.")
